...
import { FeaturesCallbacks } from './index'
...
export class ClientHandler {
   ...
   * 执行函数
   */
  public static executeFn(fn: Function | undefined | null, httpResponse: HttpResponse, logger?: Logger) {
    if (fn !== undefined && fn !== null) {
      if (typeof fn === 'function') {
        fn(httpResponse)
      } else {
        logger?.error('execute function error, `fn` is not a `function` !')
      }
    }
  }
...
export class Client {
  ...
  public async send(
    ...
    if (!ClientHandler.isSupport()) {
      ...
      ClientHandler.executeFn(props.failed, response, logger)
      return response
    }
    ...
    const getITFEValues = featuresCallbacks?.getITFEValues ?? this.getITFEValues

    return await this.getResponse(requestProps, response, getITFEValues, fetchProps, logger)
  }
  ...
  public async getResponse(
    ...
    getITFEValues: () => undefined | string[],
    fetchProps?: IHttpRequestFetchProps | undefined,
    logger?: Logger
  ): Promise<HttpResponse> {
    // fetch options
    ...
    if (timeout !== FOREVER_TIMEOUT) {
      ...
      timeoutId = setTimeout(() => {
        ...
        ClientHandler.executeFn(props.failed, httpResponse, logger)
        logger?.error(`Request aborted due to timeout: ${timeout}s !`)
      }, timeout * 1000)
    }
    ...
    const itfes = getITFEValues?.() || []
    if (itfes.length > 0) {
      // Add ITFE value as query string when set
      for (let itfe of itfes) {
    ...
    // request
    let request: Request = new Request(url, opts)
    ...
    try {
      ...
      if (props.responseType === Type.JSON) {
        try {
          ...
          logger?.error('Error while get response:', err)
      ...
      ClientHandler.executeFn(isSuccess ? props.success : props.failed, httpResponse, logger)
      return httpResponse
    } catch (err: any) {
      logger?.error(`Error while get response: ${err.message}`)
